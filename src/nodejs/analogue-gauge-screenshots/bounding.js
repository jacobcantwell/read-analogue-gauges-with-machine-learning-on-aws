...
async function getManifestLabelJson(s3Bucket, s3Key, page, oil_well_box, bp) {
  ...
  for (let i = 0; i < gauges.length; i++) {
    const gauge_box = await gauges[i].boundingBox()
    console.log('gauge_box', gauge_box)
    const label = 'GAUGE_PRESSURE_BP_' + (bp+i)
...
async function generateImageAndSaveS3(page, bp, transformSuffix, clipX, clipY) {
  ...
  try {
    ...
    */
    ...
    manifestLabels.push(getManifestLabelJson(s3Bucket, s3Key, page, oil_well_box, bp))
    // push to results
...
exports.handler = async (event, context, callback) => {
  ...
  try {
    ...
    for (let i = 10; i < 11; i = i+5) {
        let paddedBp = zeroPad(i, 3);
        let bp = zeroPad(i, 3);
        const url = URL_GAUGE_BUILDER_PREFIX + 'bp'+ zeroPad(bp++, 3)
        ...
        // original image
        await generateImageAndSaveS3(page, paddedBp, '-original', 155, 98)
        // rotate image 45
